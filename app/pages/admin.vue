  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [categoryData, productData] = await Promise.all([
        ...
        request<MarketplaceProduct[] | PaginatedResponse<MarketplaceProduct>>(
          '/products',
        ),
      ])
      categories.value = categoryData
      products.value = Array.isArray(productData)
        ? productData
        : productData.data
...
<template>
  <main class="min-h-screen bg-[#f6f8fd] px-4 py-6 text-slate-900">
    <div class="mx-auto max-w-7xl">
      <header
        class="mb-6 flex flex-col gap-4 rounded border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between"
      >
        <div>
          <NuxtLink
            to="/"
        ...
        <button
          class="h-10 rounded border border-slate-300 px-4 text-sm font-semibold hover:bg-slate-50"
          @click="logout"
        >
          Sair
        </button>
      ...
      <p
        v-if="status"
        class="mb-4 rounded border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700"
      >
        {{ status }}
      </p>
      <p
          ...
          >
            ...
            <label class="mt-4 block">
              ...
              <input
                ...
                required
                class="mt-1 h-10 w-full rounded border border-slate-300 px-3 outline-none focus:border-[#d72d91]"
              />
            </label>
            <div class="mt-4 flex gap-2">
              <button
                class="h-10 rounded bg-[#d72d91] px-4 text-sm font-semibold text-white hover:bg-[#bf247e]"
                :disabled="saving"
              >
              ...
              <button
                ...
                type="button"
                class="h-10 rounded border border-slate-300 px-4 text-sm font-semibold"
                @click="resetCategory"
              >
                Cancelar
              </button>
            ...
            <div class="mt-4 space-y-3">
              ...
              <label class="block">
                ...
                <textarea
                  ...
                  rows="3"
                  class="mt-1 w-full rounded border border-slate-300 px-3 py-2 outline-none focus:border-[#d72d91]"
                ></textarea>
              </label>
            </div>
            <div class="mt-4 flex gap-2">
            ...
            <div class="mt-4 divide-y divide-slate-100">
              ...
              >
                ...
                <div class="flex gap-2">
                  <button
                    class="rounded border border-slate-300 px-3 py-1.5 text-sm font-semibold"
                    @click="editCategory(category)"
                  >
                    Editar
                  </button>
                  <button
                    class="rounded border border-red-200 px-3 py-1.5 text-sm font-semibold text-red-600"
